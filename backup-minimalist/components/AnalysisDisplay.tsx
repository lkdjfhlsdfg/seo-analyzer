...
import { ChevronDownIcon, ChevronUpIcon, ClipboardIcon, ArrowRightIcon } from '@heroicons/react/24/outline';
...
export default function AnalysisDisplay({ analysisData }: AnalysisDisplayProps) {
  ...
  const handleViewDetails = () => {
    ...
    localStorage.setItem('analysisData', JSON.stringify(analysisData.audits));
      ...
      <div className="bg-white rounded-xl shadow-lg border border-slate-200 overflow-hidden">
        <div className="p-6">
          <div className="flex items-center justify-between mb-4">
            <h3 className="text-xl font-semibold text-slate-900">Overall Analysis Results</h3>
            <button
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
            {Object.entries(analysisData.scores).map(([category, score]) => (
              <div key={category} className="p-4 bg-slate-50 rounded-lg">
                ...
                <div className={`mt-1 text-2xl font-bold ${getScoreColor(score)}`}>
                  {score}
                </div>
              </div>
            ))}
