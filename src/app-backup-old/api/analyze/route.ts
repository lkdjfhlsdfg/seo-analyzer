...
export async function POST(req: Request) {
  try {
    ...
    if (!PAGESPEED_API_KEY) {
      ...
      return NextResponse.json(
        { error: 'PageSpeed API key is not configured. Please check your environment variables.' },
        { status: 503 }
      );
    }

    console.log('Using PageSpeed API key:', PAGESPEED_API_KEY.substring(0, 8) + '...');
    ...
    console.log('Fetching PageSpeed data from:', apiUrl);
      ...
      if (!response.ok) {
        ...
        if (response.status === 403) {
          ...
          if (errorJson.error?.message?.includes('API has not been used') || 
              errorJson.error?.message?.includes('disabled')) {
            return NextResponse.json(
              { 
                error: 'The PageSpeed Insights API is not enabled. Please enable it in the Google Cloud Console.',
          ...
          return NextResponse.json(
            { 
              error: 'Authentication error with PageSpeed API.',
              details: errorJson.error?.message || 'Please check your API key configuration.'
            },
            { status: 403 }
          );
        }
        ...
        return NextResponse.json(
          { 
            ...
            details: 'There was an error communicating with the PageSpeed API. Please try again later.'
          },
          { status: response.status }
        );
      }
